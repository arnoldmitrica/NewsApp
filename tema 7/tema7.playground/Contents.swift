...
func search(wordul: String, index: String.Index, numberOfLoops:Int) -> Int{
    ...
    print("word before \(word) cu word.first \(word.first)")
    if (wordu[position] == word.first && wordu.first != word.first){
        ...
        print("am ajuns la lastworduposition")
        print(word[index])
        print(wordu.last)
        print(wordu.first)

        if(word[index] == wordu.last && wordu.first == wordu.last){
            if let w = wordu.last {
    ...
    let lastwLetter = wordu[position]
...
//
                print("wordu[position]: \(wordu[position])")
                var w = wordu[position...endcharindex]
                print("w: \(w)")
                print("word[nextwordind] \(word[nextwordind])")
                if let nextworduindex = w.firstIndex(of: word[nextwordind]) {
                    ...
                    if position > nextworduindex{
                        ...
                        wordnow = String(wordu[position..<nextworduindex])
                        w.removeFirst(wordnow.count)
                        print("w now: \(w)")
                        print("wordappend now \(wordappend)")
                        var cpywordnow = wordnow
                        cpywordnow.removeAll(where: { !($0 == lastLetter) })
